...
from datasets.SOD import make_SOD_transforms
...
def get_images(in_path):
    ...
    for (dirpath, images, filenames) in os.walk(in_path):
...
def infer(images_path, model, postprocessors, device, output_path):
    model.eval()
    duration = 0
    for SOD_test in images_path:
        filename = os.path.basename(SOD_test)
        print("processing...{}".format(filename))
        orig_image = Image.open(SOD_test).convert('RGB')
        w, h = orig_image.size
        transform = make_SOD_transforms("val")
        dummy_target = {
